    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
	...
	if (whether_detect_object)
	{
		...
		if (!whether_read_offline_cuboidtxt)
		{
			...
			{
				std::string save_object_pose_txt =  + "/slam_output/orb_live_pred_objs_temp.txt";
	...
	std::string truth_pose_txts = base_folder + "/pose_truth.txt";
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
	...
	if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
	{
		...
		{ // read (truth) depth /stereo image for first frame
			...
			std::string right_kitti_img_file = base_data_folder + "/image_1/000000.png";
...
void Tracking::CreateInitialMapMonocular()
{
	...
	if (whether_detect_object)
	{
		DetectCuboid(pKFini);std::cout << "before error" << std::endl;
...
void Tracking::ReadAllObjecttxt()
{
	...
	for (int img_couter = 0; img_couter < total_img_ind; img_couter++)
	{
        ...
        std::string base_folder = "../data_folder";
		pred_frame_obj_txts = base_folder + "/pred_3d_obj_matched_txt/"+frame_index_c+"_3d_cuboids.txt";
...
void Tracking::DetectCuboid(KeyFrame *pKF)
{
	...
	else
	{
		...
		char frame_index_c[256];
		sprintf(frame_index_c, "%06d", (int)pKF->mnFrameId); // format into 6 digit

        //yolo
        std::vector<detect_result> results;
        std::string k_detect_model_path = "/home/robertkidder/DeepSORT/build/yolov5s.onnx";
        detect2d->init(k_detect_model_path);
        cv::Mat imDetect = pKF->raw_img.clone();
        cv::cvtColor(imDetect,imDetect,cv::COLOR_GRAY2BGR);
        detect2d->detect(imDetect,results);    

		// read detected edges

		// read yolo object detection
        typedef Eigen::Matrix<double,1,5> Vect_inv;
        std::vector<Vect_inv> raw_all_obj2d_bbox;
        for (auto result:results)
        {
            Vect_inv vec_result;
            if(result.classId==2 && result.confidence > 0.7)
            {
                vec_result[0] = result.box.x;
                vec_result[1] = result.box.y;
                vec_result[2] = result.box.width;
                vec_result[3] = result.box.height;
                vec_result[4] = result.confidence;
                raw_all_obj2d_bbox.push_back(vec_result);
            }
        }
        results.clear();
		...
		// if (!read_obj_detection_txt(data_yolo_obj_dir + frame_index_c + ".txt", raw_all_obj2d_bbox, object_classes))
		// 	std::cout << "Cannot read yolo txt  " << data_yolo_obj_dir + frame_index_c + ".txt" << std::endl;
        ...
        for (int i = 0; i < raw_all_obj2d_bbox.size(); i++)
        {
        ...
        for (size_t i = 0; i < good_object_ids.size(); i++)
        {
            ...
            for (int j=0;j<4;j++)
            {
                all_obj2d_bbox_infov_mat(i,j)=(raw_all_obj2d_bbox[good_object_ids[i]][j]);
                vec_bbox[j]=raw_all_obj2d_bbox[good_object_ids[i]][j];
            }
            all_obj2d_bbox.push_back(vec_bbox);
		...
		detect_cuboid_obj->detect_cuboid(pKF->raw_img, cam_transToGround.cast<double>(), all_obj2d_bbox_infov_mat, all_obj_cubes);
	}
