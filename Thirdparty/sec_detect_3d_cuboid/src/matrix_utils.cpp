...
void sort_indexes(const Eigen::VectorXd &vec, std::vector<int> &index, int top_k)
{
    std::partial_sort(index.begin(), index.begin() + top_k, index.end(), [&vec](int i1, int i2) { return vec(i1) < vec(i2); });
}

void sort_indexes(const Eigen::VectorXd &vec, std::vector<int> &index)
{
